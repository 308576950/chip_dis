...
def write_oneday_pricetable(iitem, row_list_tables, date, sum_df, initial_info):
    ...
    #    pdb.set_trace()
        ...
        if str(row[0][1]).replace("-", "") == date:  # str(row[0][1]) == "20161124"
            ...
            tmp_ddf = sum_df.loc[sum_df['SecurityID'] == iitem]  # 直接从sum_df中切片索引得到该股票的ddf
            ...
            #pdb.set_trace()
            sql_insert_pvtable = "insert into %s (Tra_Date,Chip, pre_p, sup_p, close) values('%s','%s',0.0, 0.0, 0.0)" % (table_name, date, today_pvtable)    # 增加close字段的赋值
            try:
        ...
        sql_cre_table = "create table if not exists %s(id int auto_increment primary key,Tra_Date date not null unique, Chip mediumtext not null, pre_p double, sup_p double, close float)" % table_name #增加float字段 增加Tra_Date为unique 
        # 还需要变Tra_Date为unique
        ...
        try:
            ...
            #sql_insert_pvtable = "insert into %s (Tra_Date,Chip) values('%s','%s')" % (
            #    table_name, date, today_pvtable)
            sql_insert_pvtable = "insert into %s (Tra_Date,Chip, pre_p, sup_p, close) values('%s','%s',0.0, 0.0, 0.0)" % (table_name, date, today_pvtable)
            cur.execute(sql_insert_pvtable)
            conn.commit()
            print(date, table_name, " cal done")
        except Exception as e:
...
def new_write_oneday_pricetable(sum_df, date, db):
    records_zb = []
    records_zxb = []
    records_cyb = []
    ...
    for item in set(sum_df["SecurityID"]):  # 代码集合
        ...
        if code_name[0] == '6':
            records_zb.append((code_name, date, str(today_pvtable)))
        if code_name[0] == '0':
            records_zxb.append((code_name, date, str(today_pvtable)))
        if code_name[0] == '3':
            records_cyb.append((code_name, date, str(today_pvtable)))
    ...
    try:
        ...
        #cur.executemany("insert into pricetable_zb (code, tra_date, chip) values(%s,%s,%s)", records_zb)
        #cur.executemany("insert into pricetable_zxb (code, tra_date, chip) values(%s,%s,%s)", records_zxb)
        #cur.executemany("insert into pricetable_cyb (code, tra_date, chip) values(%s,%s,%s)", records_cyb)
