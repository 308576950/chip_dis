...
def cal_chip_concentration(table_name, date):
    ...
    #conn = pymysql.connect(host='10.88.4.78', user='root', passwd='passw0rd', db="pv_table_backup", port=3306, charset='utf8')
    cur = conn.cursor()
    ...
    tra_date = tra_date[0:4] + '-' + tra_date[4:6] + '-' + tra_date[6:]
    for row in rows:
        code = row[0]
        tmp_chip = eval(row[1])
        weight_price = []
...
if __name__ == '__main__':
    ...
    for table in  code_table:
        ...
        sql_get_tables_from_table = "select distinct tra_date from %s where tra_date>'20180530'"%table
...
#        pdb.set_trace()
    #df.to_csv('/data/write_mysql_20180325/re_write/chip_concentration_after_20180323.csv') 
    df.to_csv('./choumayinzi_20180531.csv') 
