...
def write_oneday_pricetable(iitem, cur, conn, row_list_tables, date, sum_df, initial_info):
    code_name = str(iitem)[1:len(str(iitem))]  # 1600000 -> "600000"
    if code_name == "601313":
        code_name = "601360"
    # 由ddf计算pv_table
...
if __name__ == '__main__':
    ...
    for item in files_name[1:2]:          # 一个pricetable是一个循环，一次计算完一个pricetable
        print(item)
        ...
        for iitem in list(set(sum_df["SecurityID"])):  # 代码集合
            write_oneday_pricetable(iitem, cur, conn, row_list_tables, date, sum_df, initial_info)
            # executor.map(write_oneday_pricetable, iitem, cur, conn, row_list_tables, date, sum_df, initial_info)
            # code_name = str(iitem)[1:len(str(iitem))]    # 1600000 -> "600000"
            # ddf = sum_df.loc[sum_df['SecurityID'] == iitem]   #   直接从sum_df中切片索引得到该股票的ddf
            # if len(ddf):   # 如果存在记录
            #     #pool.apply_async(write_pv_table, (code_name, date, ddf))
